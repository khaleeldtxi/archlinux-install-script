...
pacstrap /mnt base base-devel ${kernel} ${microcode} ${kernel}-headers linux-firmware grub grub-btrfs sudo networkmanager iptables-nft efibootmgr nano zram-generator reflector bash-completion btrfs-progs os-prober git curl apparmor terminus-font snapper snap-pac zsh zsh-doc grml-zsh-config zsh-autosuggestions zsh-completions zsh-history-substring-search zsh-syntax-highlighting zsh-lovers zsh-theme-powerlevel10k powerline firewalld dosfstools sysfsutils usbutils e2fsprogs vim sddm which tree pipewire python-pip python-setuptools xorg-server-devel plasma wireless_tools wpa_supplicant dolphin dolphin-plugins ark konsole kcalc ffmpegthumbs libreoffice-fresh plasma-wayland-session qt5-wayland qt6-wayland bluez mtools inetutils less man-pages texinfo python-psutil pipewire-pulse pipewire-alsa pipewire-jack flatpak adobe-source-han-sans-otc-fonts adobe-source-han-serif-otc-fonts gnu-free-fonts bluez-utils xdg-utils xdg-user-dirs ntfs-3g neofetch wget openssh cronie htop p7zip mlocate man-db wireplumber firefox qemu virt-manager ebtables qemu-arch-extra edk2-ovmf dnsmasq bridge-utils swtpm chrony libvirt konsole kate gwenview kcron kdeconnect kdegraphics-thumbnailers kdenetwork-filesharing kdenlive kdesdk-thumbnailers kdf kfind kio-admin kio-extras kio-gdrive kio-zeroconf ksystemlog okular yakuake podman docker distrobox --noconfirm --needed
